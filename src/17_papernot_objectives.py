...
def run(
    ...
    # ----- Predict
    y_attack_proba = model.predict_proba(X_attacks)
    y_pred_proba = model.predict_proba(X_initial_states)
    ...
    # Constraints (O3)
    constraints = venus_constraints.evaluate(X_misclassified)
    ...
    # Distance
    distances = np.linalg.norm(scaler.transform(X_attacks)-scaler.transform(X_initial_states), axis=1)
